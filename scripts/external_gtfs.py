...
def build_external_stop_packages(
    ...
    stops_by_city_id: dict[str, list[dict[str, object]]] = {
    ...
    for stop in swiss_stops(iter_table(archive, "stops.txt")):
        for city in cities:
            if distance_meters(
                ...
                stops_by_city_id[str(city["id"])].append(dict(stop))
    ...
    for city in cities:
        city_id = str(city["id"])
        city_stops = stops_by_city_id[city_id]
        if not city_stops:
        ...
        package_stops[city_id] = city_stops
        manifest.append({
            "id": city["id"],
            "name": city["name"],
...
def build_external_route_index(
    ...
    for city in cities:
        ...
        city_stop_ids = {str(stop["id"]) for stop in city_stops}
...
def build_external_departure_index(
    ...
    all_included_stop_ids: set[str] = set()
    for city in cities:
        city_id = str(city["id"])
        stop_path = packages_directory / f"{city_id}.json"
        if not stop_path.exists():
    ...
    for stop_time in iter_table(archive, "stop_times.txt"):
        ...
        if stop_id and departure_time and stop_id in all_included_stop_ids:
            stop_departures.setdefault(stop_id, []).append((departure_time, trip_id))
        try:
    ...
    for city in cities:
        ...
        for stop_id in sorted(city_stop_ids.get(city_id, set())):
            ...
            for departure_time, trip_id in sorted(
                ...
                key = (trip_id, route_id, destination, direction_id, departure_time)
                if key in seen:
                    continue
